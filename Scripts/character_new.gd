...
const FRICTION = 13
const HORIZONTAL_ACCELERATION = 18
const MAX_SPEED = 8.0
...
func _physics_process(delta):
	...
	if Input.is_action_just_pressed("ui_accept") and is_on_floor() and Input.mouse_mode==Input.MOUSE_MODE_CAPTURED:
		...
		print("Player height is:", global_position)

	# As good practice, you should replace UI actions with custom gameplay actions.
	var input_dir = Vector3.ZERO
	var movetoward = Vector3.ZERO
	input_dir.x = Input.get_vector("move_left", "move_right", "move_up", "move_down").x
	input_dir.y = Input.get_vector("move_left", "move_right", "move_up", "move_down").y
